...
document.addEventListener("keydown", function (event) {
  if (event.key === "Tab") {
    ...
    if (!noButton.dataset.tabPressed) {
      ...
      // "title" change
      const title = document.querySelector(".title");
      title.textContent = "Kalbimi çaldın zaten birde burda hile yapma :)";

      //  turn back
      setTimeout(() => {
        title.textContent = "Evlenelim mi artık Simav?";
      }, 2000);
    }
  }
});
